---
description: Connect a Solana validator (mainnet-beta or testnet) and up to three backups to DoubleZero in IBRL mode, including identity proof and the connection request.
---

# Validator Connection in IBRL Mode
...
Pick the DoubleZero network that matches your Solana cluster: `mainnet-beta` or `testnet`. Install the matching packages in [setup](setup.md), and use that same network for every command below.

!!! Note inline end
    IBRL mode does not require restarting validator clients, because it uses your existing public IP address.

Solana validators connect to DoubleZero in IBRL mode using the steps on this page.
...
Confirm the client is on the network you chose:

```bash
doublezero status
```

The `Network` column should be `mainnet-beta` or `testnet`, matching your Solana cluster. If it is wrong, or you installed the wrong package, use the copy-paste switch in [troubleshooting](troubleshooting.md#issue-wrong-doublezero-environment).
...
Example output (mainnet-beta; testnet looks the same but with fewer devices):
...
This output can be used to see the transaction on a Solana explorer. Be sure to set the explorer to mainnet-beta or testnet to match your cluster. This verification is optional.
...
(`Network` shows `testnet` when you connected on testnet.)
