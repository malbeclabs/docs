---
description: doublezerod를 설치하고 검증자 또는 노드를 DoubleZero 네트워크에 연결하는 단계별 가이드입니다.
---
...
- DoubleZero를 실행할 서버의 root 또는 sudo 권한
...
DoubleZero에 온보딩할 때 **DoubleZero ID**라는 공개 키로 표현되는 **DoubleZero 신원**을 생성합니다. 이 키는 DoubleZero가 여러분의 머신을 인식하는 방식의 일부입니다.
...
??? info "기존 사용자 전용: *Testnet에서 Mainnet-Beta로*, 또는 *Mainnet-Beta에서 Testnet으로* 패키지 변경"
    ...
    Mainnet-Beta에서 Testnet으로 이동하는 경우에도 동일한 단계를 수행할 수 있으며, 3단계를 위의 Testnet용 설치 명령어로 대체하면 됩니다.


    1. 기존 저장소 파일 찾기

        먼저 시스템에서 기존 DoubleZero 저장소 구성 파일을 찾으세요:
    ...
    2. 기존 저장소 파일 제거
        ...
        이전 단계에서 찾은 기존 저장소 파일을 제거합니다. 예시:
    ...
    3. 새 저장소에서 설치
        ...
        새 Mainnet-Beta 저장소를 추가하고 최신 패키지를 설치합니다:
...
DoubleZero는 GRE 터널링(IP 프로토콜 47)과 BGP 라우팅(링크-로컬 주소에서 tcp/179)을 사용합니다. 방화벽에서 이러한 프로토콜을 허용하는지 확인하세요:
...
다음 명령어로 서버에서 DoubleZero 신원을 생성합니다:
...
!!! info
    사용하려는 기존 ID가 있는 경우 다음 선택 사항 단계를 따를 수 있습니다.
    ...
    DoubleZero에 사용할 `id.json`을 doublezero 구성 디렉토리에 복사하거나 링크합니다.
...
DoubleZero 신원을 확인합니다. 이 신원은 여러분의 머신과 DoubleZero 간의 연결을 생성하는 데 사용됩니다.
...
연결하기 전에 `doublezerod`가 사용 가능한 모든 DZ 테스트넷 스위치를 발견하고 핑했는지 확인하세요:
...
출력에 장치가 표시되지 않으면 10-20초 기다린 후 다시 시도하세요.

## 5. DoubleZero 연결 해제

다음 섹션에서 DoubleZero 환경을 설정합니다. 성공을 보장하기 위해 현재 세션을 연결 해제하세요. 이렇게 하면 머신에서 여러 터널이 열려 있는 것과 관련된 문제를 방지할 수 있습니다.
...
DoubleZero 연결은 사용 사례에 따라 다릅니다. DoubleZero에서 테넌트는 유사한 사용자 프로필을 가진 그룹입니다. 예시로는 블록체인, 데이터 전송 레이어 등이 있습니다.
...
Prometheus 메트릭에 익숙한 운영자는 DoubleZero 모니터링을 위해 메트릭을 활성화할 수 있습니다. 이를 통해 DoubleZero 클라이언트 성능, 연결 상태 및 운영 건전성에 대한 가시성을 제공합니다.
...
시각화를 위해 DoubleZero 메트릭을 사용하여 Grafana 대시보드를 만들 수 있습니다. 일반적인 패널에는 다음이 포함됩니다:
- 시간에 따른 세션 상태
- 빌드 정보
- 연결 지연 시간 추세
- 오류율 모니터링