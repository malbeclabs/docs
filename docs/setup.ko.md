---
description: doublezerod를 설치하고 밸리데이터 또는 노드를 DoubleZero 네트워크에 연결하는 단계별 가이드입니다.
---
...
DoubleZero에 온보딩할 때 **DoubleZero ID**라고 하는 공개 키로 표현되는 **DoubleZero 신원**을 생성하게 됩니다. 이 키는 DoubleZero가 여러분의 머신을 인식하는 방법의 일부입니다.
...
??? info "기존 사용자만 해당: 패키지를 *Testnet에서 Mainnet-Beta로* 또는 *Mainnet-Beta에서 Testnet으로* 변경"
    위의 패키지 저장소 중 하나에서 설치하면 DoubleZero **Testnet** 또는 **DoubleZero Mainnet Beta**에 특화됩니다. 네트워크를 전환하는 경우 이전에 설치된 패키지 저장소를 제거하고 대상 저장소로 업데이트해야 합니다.

    이 예제는 Testnet에서 Mainnet-Beta로의 마이그레이션 과정을 안내합니다.

    Mainnet-Beta에서 Testnet으로 이동하는 경우에도 동일한 단계를 완료할 수 있으며, 3단계를 위의 Testnet 설치 명령으로 대체하면 됩니다.
...
패키지가 설치된 후 새로운 systemd 유닛이 설치, 활성화 및 시작됩니다. 상태를 확인하려면 다음을 실행하세요:
```
...
DoubleZero는 GRE 터널링(IP 프로토콜 47)과 BGP 라우팅(링크-로컬 주소의 tcp/179)을 사용합니다. 방화벽에서 이 프로토콜들을 허용하는지 확인하세요:
...
!!! info
    ...
    DoubleZero에 사용할 `id.json`을 doublezero 설정 디렉토리로 복사하거나 링크하세요.
...
DoubleZero 신원을 확인하세요. 이 신원은 여러분의 머신과 DoubleZero 간의 연결을 생성하는 데 사용됩니다.
...
## 4. doublezerod가 DZ 장치를 검색했는지 확인

연결하기 전에, `doublezerod`가 사용 가능한 모든 DZ 테스트넷 스위치를 검색하고 핑했는지 확인하세요:
...
출력에 장치가 반환되지 않으면 10-20초 대기 후 다시 시도하세요.

## 5. DoubleZero 연결 해제

다음 섹션에서 DoubleZero 환경을 설정합니다. 성공을 보장하기 위해 현재 세션의 연결을 해제하세요. 이렇게 하면 머신에서 여러 터널이 열려 있는 것과 관련된 문제를 방지할 수 있습니다.
...
DoubleZero에 대한 연결은 사용 사례에 따라 다릅니다. DoubleZero에서 테넌트는 유사한 사용자 프로필을 가진 그룹입니다. 블록체인, 데이터 전송 레이어 등이 그 예입니다.
...
Prometheus 메트릭에 익숙한 운영자는 DoubleZero 모니터링을 위해 이를 활성화할 수 있습니다. 이를 통해 DoubleZero 클라이언트 성능, 연결 상태 및 운영 건전성에 대한 가시성을 확보할 수 있습니다.

## 사용 가능한 메트릭

DoubleZero는 몇 가지 주요 메트릭을 노출합니다:
- **빌드 정보**: 버전, 커밋 해시, 빌드 날짜
...
`-env` 플래그는 데이터를 수집하려는 네트워크에 따라 `testnet` 또는 `mainnet-beta`를 가리켜야 합니다. 아래 샘플 블록에서는 `testnet`이 사용되었습니다. 필요한 경우 `mainnet-beta`로 변경할 수 있습니다.
...
Grafana는 시각화를 위한 훌륭한 옵션이며, Prometheus 메트릭을 수집하는 방법을 자세히 설명하는 문서가 [여기](https://grafana.com/docs/alloy/latest/collect/prometheus-metrics/)에 있습니다.
