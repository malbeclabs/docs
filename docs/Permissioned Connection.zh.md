---
description: 非验证者和 RPC 以 IBRL 模式连接到 DoubleZero Mainnet-Beta 和 Testnet 的许可制接入。
---
...
目前，非验证者和 RPC 的用户接入采用许可制。如需启动许可制流程，请填写[此表单](https://forms.fillout.com/t/s77k7wandMus?id=rec08iF4Z8kVFGm1z)。以下是此流程中需要了解的事项：

- 未来许可制用户的使用可能会产生费用。
- 提交表单后，请关注您的 Telegram 主要联系方式。


### 连接到 Mainnet-Beta 和 Testnet 的 IBRL 模式

!!! Note inline end
    IBRL 模式不需要重启验证者客户端，因为它使用您现有的公共 IP 地址。

许可制用户将完成到 DoubleZero Mainnet-beta 的连接，本页面将详细介绍该流程。

## 1. 确认客户端网络

请在继续之前按照[设置](setup.md)说明进行操作。为您想要加入的网络安装 Mainnet-Beta 或 Testnet 软件包——它们使用不同的软件包仓库。
...
`Network` 列应与您打算加入的网络匹配。如果不匹配，请使用[故障排除](troubleshooting.md#issue-wrong-doublezero-environment)中的复制粘贴切换方法。
...
联系 DoubleZero 基金会。您需要提供您的 `DoubleZeroID`、您的 `Validator ID`（节点 ID）以及您将要连接的`公共 ipv4 地址`。


## 3. 以 IBRL 模式连接

在服务器上，使用将要连接到 DoubleZero 的用户，运行 `connect` 命令来建立与 DoubleZero 的连接。
...
✅  User Provisioned
```
请等待一分钟让隧道完成建立。在隧道完成之前，您的状态输出可能会返回 "down" 或 "Unknown"。
