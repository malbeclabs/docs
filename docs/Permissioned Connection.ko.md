---
description: IBRL 모드에서 DoubleZero Mainnet-Beta 및 Testnet에 연결하는 비검증인 및 RPC를 위한 허가형 온보딩.
---

# IBRL 모드에서 DoubleZero에 대한 비검증인 허가형 연결
!!! warning "DoubleZero에 연결함으로써 [DoubleZero 서비스 약관](https://doublezero.xyz/terms-protocol)에 동의합니다"

### 허가형 사용자 온보딩 개요

현재 비검증인 및 RPC에 대한 사용자 온보딩은 허가형으로 운영됩니다. 허가형 절차를 시작하려면 [이 양식](https://forms.fillout.com/t/s77k7wandMus?id=rec08iF4Z8kVFGm1z)을 작성해 주세요. 이 과정에서 예상할 수 있는 사항은 다음과 같습니다:

- 향후 허가형 사용자 이용에 대해 수수료가 부과될 수 있습니다.
...
!!! Note inline end
    IBRL 모드는 기존 공개 IP 주소를 사용하기 때문에 검증인 클라이언트를 재시작할 필요가 없습니다.

허가형 사용자는 이 페이지에서 자세히 설명하는 DoubleZero Mainnet-beta 연결을 완료하게 됩니다.

## 1. 클라이언트 네트워크 확인

진행하기 전에 [설정](setup.md) 지침을 따라 주세요. 원하는 네트워크에 맞는 Mainnet-Beta 또는 Testnet 패키지를 설치하세요 — 서로 다른 패키지 저장소를 사용합니다.

설정의 마지막 단계는 네트워크에서 연결을 해제하는 것이었습니다. 이는 DoubleZero에 대한 터널이 머신에서 하나만 열려 있고, 해당 터널이 올바른 네트워크에 연결되어 있는지 확인하기 위함입니다.
...
```
Testnet 출력은 구조가 동일하지만, 사용 가능한 장치가 훨씬 더 많습니다.
...
✅  User Provisioned
```
터널이 완료될 때까지 1분간 기다려 주세요. 터널이 완료되기 전까지 상태 출력이 "down" 또는 "Unknown"을 반환할 수 있습니다.
...
이 설정을 완료했으며 멀티캐스트를 사용할 계획이라면, [다음 페이지](Other%20Multicast%20Connection.md)로 진행하세요.