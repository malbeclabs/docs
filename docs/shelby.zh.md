...
Shelby 许可用户将完成与 DoubleZero 测试网的连接，本页面对此进行了详细说明。

## 1. 环境配置

请在继续之前按照[设置](setup.md)说明进行操作。

设置的最后一步是断开与网络的连接。这是为了确保您的机器上只有一个通往 DoubleZero 的隧道处于打开状态，并且该隧道连接到正确的网络。

要配置 DoubleZero Client CLI (`doublezero`) 以连接到 DoubleZero 上的 Shelby 租户：
```bash
doublezero config set --tenant shelby
```

应用 Shelby 特定的额外防火墙规则：
...
联系 DoubleZero 基金会。您需要提供您的 `DoubleZeroID` 以及您将要从其连接的 `public ipv4 address`。


## 3. 以 IBRL 模式连接

在服务器上，使用将要连接 DoubleZero 的用户，运行 `connect` 命令以建立与 DoubleZero 的连接。
