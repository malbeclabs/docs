...
Si vous avez déjà configuré DoubleZero, assurez-vous de disposer de la dernière version de la CLI Doublezero-Solana avec `sudo apt update && sudo apt install doublezero-solana`
...
### 4. Récupérer l'identité DoubleZero du serveur
...
Ceci écrit dans `~/.config/solana/id.json`. Pour utiliser un chemin différent, passez `--keypair <path>` à n'importe quelle commande `doublezero-solana`.
...
Votre portefeuille nécessite deux tokens :

- **SOL** — pour les frais de transaction Solana. Transférez des SOL à l'adresse du portefeuille affichée ci-dessus.
- **USDC** — pour le financement des sièges. La CLI prélève depuis le compte de token associé (ATA) de votre portefeuille pour le mint USDC mainnet (`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`).
...
Notez le code de l'appareil du résultat ayant la latence la plus faible (par ex., `<Device_Name>`). Vous l'utiliserez lors de l'achat d'un siège.
...
Achetez un siège avec une seule commande. Ceci initialise votre siège, approvisionne l'escrow et demande l'attribution :
...
| `--amount <USDC>` | USDC à déposer (format décimal, par ex. `100` = 100 USDC). Doit respecter le prix minimum par epoch. |
...
| `--with-compute-unit-price <PRICE>` | Définir un prix d'unité de calcul pour une inclusion plus rapide en période de congestion |
...
Les sièges sont attribués par epoch Solana (~2 jours). S'il reste moins de 10 % de l'epoch en cours lorsque vous payez, la CLI vous avertit que votre siège sera attribué immédiatement mais ne couvrira que le reste de l'epoch en cours. Un paiement séparé sera déduit de votre escrow au début de l'epoch suivant.
...
!!! warning "Si le solde de votre escrow est inférieur au prix de l'epoch lors du règlement, votre siège ne sera pas attribué, le tunnel sera démonté et vous perdrez votre ancienneté accumulée. L'ancienneté détermine votre priorité pour les epochs futurs — la perdre signifie que vous êtes à nouveau en concurrence en tant que nouveau venu."

Vous pouvez surfinancer ce compte pour couvrir plusieurs epochs. Chaque règlement déduit le prix d'un epoch de votre escrow, et le solde restant est reporté. Par exemple, financer 5x le prix par epoch maintient votre siège actif pendant 5 epochs maximum sans réapprovisionnement.

Pour réapprovisionner votre escrow, exécutez à nouveau `shreds pay` à tout moment :
...
Cette section détaille comment visualiser les sièges via la CLI. Vous pouvez également utiliser [https://data.malbeclabs.com/api/v1/docs](https://data.malbeclabs.com/api/v1/docs) pour surveiller les sièges et vous aider à gérer votre compte escrow.

Consultez vos sièges actifs et les soldes escrow :
...
Fermez votre escrow et récupérez les USDC restants dans votre portefeuille :
...
Pour envoyer le remboursement vers un compte de token différent :
...
!!! warning "Retirer des fonds signifie que vous perdez votre siège et l'ancienneté accumulée."

---

## Adresses de shreds (IP vs Port)

Les shreds Leader et les shreds Retransmit à haut stake arriveront sur le port `7733`, via l'interface `doublezero1`. L'interface `doublezero0` est destinée au trafic unicast. Le port `5765` est un moniteur de heartbeat provenant des éditeurs de shreds — il ne contiendra pas de shreds.
...
### [Tableau de bord Edge](https://data.malbeclabs.com/dz/shreds/scoreboard)

Le tableau de bord évalue la vitesse de livraison des shreds à travers DoubleZero Edge et d'autres fournisseurs, en utilisant des données au niveau des slots pour comparer les performances en temps réel. Utilisez ce tableau de bord pour voir les taux de victoire des shreds Edge par rapport aux autres fournisseurs. Vous pouvez consulter les résultats pour les shreds leader uniquement, en plus de la comparaison du flux complet. Vous pouvez également filtrer par région pour voir les performances attendues.

### [Éditeurs Edge](https://data.malbeclabs.com/dz/shreds/publishers)

La métrique « Publishing Shreds » en haut à gauche du tableau de bord montre le pourcentage total du poids de stake de tous les validateurs Solana publiant des shreds leader sur DoubleZero Edge. Vous pouvez voir les détails de chaque éditeur sur le réseau.

### [Abonnés, appareils et activité Edge](https://data.malbeclabs.com/dz/shreds/subscribers)

Vous pouvez facilement rechercher votre IP client sur cette page pour les sièges souscrits et consulter le statut. Cliquez sur des souscriptions de sièges spécifiques pour voir l'historique des paiements et l'activité. Vous pouvez également consulter les appareils disponibles sur la page [Appareils](https://data.malbeclabs.com/dz/shreds/devices) et toute l'activité récente sur la page [Activité](https://data.malbeclabs.com/dz/shreds/activity).

### Documentation de l'API de données

Pour un accès programmatique aux endpoints de données, consultez la documentation de l'API : [https://data.malbeclabs.com/api/v1/docs](https://data.malbeclabs.com/api/v1/docs).
...
### Solde escrow insuffisant

Si le solde de votre escrow est inférieur au prix de l'epoch lors du règlement, le siège n'est pas attribué, le tunnel est démonté et l'ancienneté est perdue. Réapprovisionnez avec `shreds pay` avant le prochain règlement.

### Siège non attribué après paiement

- Vous avez peut-être payé tard dans l'epoch — le siège prend effet à l'epoch suivant.
- Tous les sièges de l'appareil peuvent être occupés par des titulaires ayant une ancienneté supérieure. Vérifiez les sièges disponibles avec `shreds price`.
- Si vous avez retiré vos fonds avant le règlement, le siège n'était pas éligible.
...
L'IP client du daemon est découverte automatiquement à partir de l'IP publique de votre hôte — vérifiez qu'elle correspond au `--client-ip` utilisé dans vos commandes de siège.
...
La commande `pay` valide votre montant par rapport au prix minimum par epoch (base métro + prime appareil). Utilisez `shreds price` pour vérifier les tarifs actuels et augmentez votre montant.
