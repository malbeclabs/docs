---
description: Configura un edge subscriber per ricevere i feed di shred DoubleZero, inclusa la configurazione del client e le regole del firewall per GRE, BGP, PIM e il traffico shred.
---

# Connessione Edge Subscriber (CLI)

!!! warning "Sottoscrizione CLI legacy — disattivata il **30 agosto 2026**"
    Questa pagina è destinata agli utenti che utilizzano già la sottoscrizione **CLI / seat onchain** (`doublezero-solana shreds pay`). Questo sistema verrà disattivato il **30 agosto 2026**.

    Le nuove sottoscrizioni utilizzano il flusso tramite portale su [Sottoscrivere agli shred (Edge)](Edge Subscriber Connection.md).
...
La CLI `doublezero-solana` utilizza un keypair Solana standard per la gestione dei seat onchain. Se non ne hai uno:
...
Annota il codice del dispositivo dal risultato con la latenza più bassa (es., `<Device_Name>`). Lo utilizzerai al momento dell'acquisto di un seat.

### 2. Verificare i prezzi

Visualizza i prezzi correnti dei dispositivi prima di impegnare fondi. I prezzi hanno due componenti: un **prezzo base della metro** e un **premium per dispositivo**. Puoi anche visualizzare prezzi e disponibilità [qui](https://data.doublezero.xyz/dz/shreds/devices).
...
Il prezzo per epoca è il costo totale per epoca per un seat su quel dispositivo (base + premium). Usa `--wide` per mostrare le pubkey complete, o `--json` per l'output in formato JSON.
...
| `--amount <USDC>` | USDC da finanziare (formato decimale, es. `100` = 100 USDC). Deve soddisfare il prezzo minimo per epoca. |
| `--source-token-account <PUBKEY>` | Account sorgente USDC personalizzato (predefinito: ATA del tuo wallet) |
| `--accept-partial-epoch` | Salta l'avviso di epoca rimanente (vedi sotto) |
...
### Tempistica delle epoche

I seat vengono allocati per epoca Solana (~2 giorni). Se rimane meno del 10% dell'epoca corrente quando effettui il pagamento, la CLI avverte che il tuo seat verrà allocato immediatamente ma coprirà solo il resto dell'epoca corrente. Un pagamento separato verrà detratto dal tuo escrow quando inizia l'epoca successiva.

!!! info "È consigliabile finanziare per più di 1 epoca alla volta per non perdere il tuo seat. Puoi controllare il tempo rimanente in un'epoca [qui](https://explorer.solana.com/)."

Puoi bypassare questo avviso con `--accept-partial-epoch`.

### Mantieni il tuo escrow finanziato

!!! warning "Se il saldo del tuo escrow è inferiore al prezzo per epoca al momento del settlement, il tuo seat non verrà allocato, il tunnel verrà smantellato e perderai la tenure accumulata. La tenure determina la tua priorità per le epoche future — perderla significa competere nuovamente come nuovo arrivato."

Puoi sovraccaricare questo account per finanziare più epoche. Ogni settlement deduce il prezzo di un'epoca dal tuo escrow e il saldo rimanente viene riportato. Ad esempio, finanziare 5x il prezzo per epoca mantiene il tuo seat attivo per un massimo di 5 epoche senza rifinanziamento.

Per ricaricare il tuo escrow, esegui `shreds pay` di nuovo in qualsiasi momento:
...
Questa sezione descrive come visualizzare i seat tramite la CLI. Puoi anche usare [https://data.doublezero.xyz/api/v1/docs](https://data.doublezero.xyz/api/v1/docs) per monitorare i seat e gestire il tuo account escrow.
...
La colonna "Est. Epochs Paid" mostra quante epoche copre il tuo saldo attuale ai prezzi correnti. Se i prezzi cambiano, questa stima si aggiorna.

### Ritirare seat ed escrow

Questo comando rilascia il tuo seat e chiude l'escrow. Ricevi un rimborso proporzionale per la porzione inutilizzata dell'epoca corrente, più qualsiasi saldo escrow rimanente, restituito al tuo wallet. Perdi il seat e qualsiasi tenure accumulata.
...
!!! warning "Questa operazione non può essere annullata. Dopo il ritiro, il tuo seat viene eliminato e la tenure viene azzerata."

---

## Indirizzi shred (IP vs Porta)

I Leader Shred e i Retransmit Shred ad alto stake arriveranno sulla porta `7733`, attraverso l'interfaccia `doublezero1`. L'interfaccia `doublezero0` è per il traffico unicast. La porta `5765` è un monitor heartbeat degli shred publisher — non conterrà shred.
...
Se il saldo del tuo escrow è inferiore al prezzo per epoca al momento del settlement, il seat non viene allocato, il tunnel viene smantellato e la tenure viene persa. Ricarica con `shreds pay` prima del prossimo settlement.

### Seat non allocato dopo il pagamento

- Potresti aver pagato tardi nell'epoca — il seat diventa effettivo nell'epoca successiva.
- Tutti i seat sul dispositivo potrebbero essere occupati da incumbent con tenure più alta. Controlla i seat disponibili con `shreds price`.
- Se hai effettuato il ritiro prima del settlement, il seat non era idoneo.
...
3. Verifica che le regole del firewall siano in atto (GRE, BGP, PIM, traffico shred su `doublezero1`, porta 44880 su `doublezero0`)
4. Verifica che il tuo seat sia attivo per l'epoca corrente: `doublezero-solana shreds list`
5. Controlla lo stato della tua connessione: `doublezero status`

L'IP del client del daemon viene rilevato automaticamente dall'IP pubblico del tuo host — verifica che corrisponda al `--client-ip` utilizzato nei tuoi comandi per il seat.

### Avviso prompt dell'epoca

La CLI avverte quando rimane meno del 10% dell'epoca. Le tue opzioni sono:

- Accettare con `--accept-partial-epoch` se vuoi il seat immediatamente
- Attendere l'epoca successiva per ottenere una copertura completa dell'epoca

### "Amount is below the current price"

Il comando `pay` valida il tuo importo rispetto al prezzo minimo per epoca (base metro + premium dispositivo). Usa `shreds price` per controllare i prezzi correnti e aumenta il tuo importo.
