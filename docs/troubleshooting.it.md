...
Questa guida coprirà una varietà di problemi ed è in continuo aggiornamento. Se completi la guida puoi cercare ulteriore supporto nel discord di [DoubleZero Tech](https://discord.com/channels/1341597747932958802/1344323790464880701).


## Comandi e Output Comuni

Per iniziare, esamina l'output dei seguenti comandi e il loro output atteso. Questi ti aiuteranno nella risoluzione più dettagliata dei problemi.
...
```
[comment]: # ()
#### 4. Verifica Crediti Ledger DoubleZero
Comando:
...
3. Se quanto sopra sembra corretto e stai ricevendo un errore durante la connessione, oppure se la mappatura sopra indicata non è corretta, contatta il supporto su [DoubleZero Tech](https://discord.com/channels/1341597747932958802/1344323790464880701)
...
3. L'errore appare perché un tunnel verso DoubleZero con lo specifico IP DoubleZero è già attivo su questa macchina.
    ...
    Questo errore si incontra spesso dopo un aggiornamento del client DoubleZero. Gli aggiornamenti di DoubleZero riavviano automaticamente il servizio doublezerod e ti riconnettono se eri connesso prima del riavvio del servizio.


### Problema: Lo stato di DoubleZero è unknown o down
Questo problema è spesso legato al tunnel GRE che viene attivato con successo tra il server e il dispositivo DoubleZero, ma un firewall impedisce l'instaurazione della sessione BGP. Per questo motivo non stai ricevendo route dalla rete né inviando traffico attraverso DoubleZero.
...
1. Controlla le regole del tuo firewall!
   ...
   DoubleZero utilizza lo spazio di indirizzi link local: 169.254.0.0/16 per le interfacce del tunnel GRE tra la tua macchina e il dispositivo DoubleZero. 169.254.0.0/16 è tipicamente uno spazio "non instradabile" e quindi le buone pratiche di sicurezza raccomanderanno di bloccare le comunicazioni da/verso questo spazio. Dovrai consentire una regola nel tuo firewall che permetta alla sorgente 169.254.0.0/16 di comunicare con la destinazione 169.254.0.0/16 sulla porta tcp 179. Questa regola dovrà essere posizionata sopra qualsiasi regola che nega il traffico verso 169.254.0.0/16.
    ...
    Nell'output sopra si vede che tutto il traffico verso 169.254.0.0/16, ad eccezione delle porte specificate, è negato.
    `sudo ufw insert <N> allow proto tcp from 169.254.0.0/16 to 169.254.0.0/16 port 179` per inserire la regola nella posizione <N>. Ad esempio, se N = 1, inserirai questa regola come prima regola.
...
2. Determina se sei già connesso al dispositivo di destinazione
    - esegui `doublezero user list --env testnet | grep 111.11.11.11` sostituisci `111.11.11.11` con l'indirizzo IPv4 pubblico del tuo dispositivo connesso a DoubleZero. Puoi anche usare il tuo validator ID o doublezero ID.
        ...
        ```
        Questa sarebbe una connessione non ottimale. Consideriamo se è necessaria una riconnessione.
...
3. Opzionale: esamina la rete per i dispositivi disponibili
    ...
    - esegui `doublezero device list` per un elenco completo dei dispositivi. Abbiamo estratto 2 dispositivi come esempio per spiegare l'output.
        ...
        Tuttavia, `dz-fr5-sw01` ha 0 utenti e 0 utenti massimi. Non sarai in grado di connetterti a questo dispositivo. Con un massimo di 0 utenti, il dispositivo non accetta alcuna connessione.

        Ora torniamo a connetterci al nostro dispositivo più vicino.

4. Determina se il dispositivo di destinazione ha una connessione disponibile
    - esegui `doublezero device list | grep dz-ny7-sw01` sostituisci `dz-ny7-sw01` con il tuo dispositivo di destinazione
...
5. Connettiti al dispositivo DoubleZero più vicino
    ...
    ✅  User Provisioned
    ```
    nota nell'output sopra che ci siamo `Connected to device: dz-ny7-sw01` questo è il risultato desiderato dalla nostra indagine iniziale al passaggio 1, dove abbiamo scoperto che `dz-ny7-sw01` era il dispositivo con la latenza più bassa.
...
`doublezero status`
    ...
    Questo è indicativo di un tunnel aperto sulla tua macchina che non si trova nel tuo ambiente attuale.
