...
- 在运行 DoubleZero 的服务器上拥有 root 或 sudo 权限
...
??? info "仅限现有用户：将软件包从 *Testnet 切换到 Mainnet-Beta*，或从 *Mainnet-Beta 切换到 Testnet*"
    当您从上述某个软件包仓库安装时，它是专门针对 DoubleZero **Testnet** 或 **DoubleZero Mainnet Beta** 的。如果您在任何时候切换网络，您需要移除先前安装的软件包仓库并更新到目标仓库。

    本示例将演示从 Testnet 迁移到 Mainnet-Beta 的过程

    同样的步骤也可用于从 Mainnet-Beta 迁移到 Testnet，只需将步骤 3 替换为上方对应的 Testnet 安装命令即可。


    1. 查找旧的仓库文件

        首先，找到系统上任何现有的 DoubleZero 仓库配置文件：
...
!!! info
    如果您有一个现有的 ID 想要使用，可以按照以下可选步骤操作。
    ...
    将您想要用于 DoubleZero 的 `id.json` 复制或链接到 doublezero 配置目录。
...
连接到 DoubleZero 的方式因您的使用场景而异。在 DoubleZero 上，租户是具有相似用户配置文件的群组。例如区块链、数据传输层等。
...
熟悉 Prometheus 指标的运维人员可能希望为 DoubleZero 监控启用这些指标。这提供了对 DoubleZero 客户端性能、连接状态和运行健康状况的可见性。

## 可用的指标

DoubleZero 暴露了几个关键指标：
- **构建信息**：版本、提交哈希和构建日期
...
请注意，`-env` 标志需要指向 `testnet` 或 `mainnet-beta`，具体取决于您要从哪个网络收集数据。在示例配置中使用的是 `testnet`。如果需要，您可以将其替换为 `mainnet-beta`。
...
配置和安全性超出了本文档的范围。
Grafana 是一个出色的可视化选项，其文档可在[此处](https://grafana.com/docs/alloy/latest/collect/prometheus-metrics/)获取，详细介绍了如何收集 Prometheus 指标。

## Grafana 仪表板（可选）

对于可视化，您可以使用 DoubleZero 指标创建 Grafana 仪表板。常见的面板包括：
- 会话状态随时间变化
- 构建信息
- 连接延迟趋势
- 错误率监控