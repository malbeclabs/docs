...
??? info "Utilisateurs existants uniquement : Changer un paquet de *Testnet à Mainnet-Beta*, ou de *Mainnet-Beta à Testnet*"
    Lorsque vous installez depuis l'un des dépôts de paquets ci-dessus, il est spécifique à DoubleZero **Testnet** ou **DoubleZero Mainnet Beta**. Si vous changez de réseau à un moment donné, vous devrez supprimer les dépôts de paquets précédemment installés et mettre à jour vers le dépôt cible.
...
Si aucun appareil n'apparaît dans la sortie, attendez 10 à 20 secondes et réessayez.

## 5. Se déconnecter de DoubleZero

Dans les sections suivantes, vous configurerez votre environnement DoubleZero. Pour garantir le succès, déconnectez la session en cours. Cela évitera les problèmes liés à l'ouverture de plusieurs tunnels sur votre machine.
...
- **Statut de la session** : Indique si la session DoubleZero est active
...
Grafana est une excellente option de visualisation et dispose d'une documentation disponible [ici](https://grafana.com/docs/alloy/latest/collect/prometheus-metrics/) détaillant comment collecter les métriques Prometheus.

## Tableau de bord Grafana (Optionnel)

Pour la visualisation, vous pouvez créer un tableau de bord Grafana utilisant les métriques DoubleZero. Les panneaux courants incluent :
- Statut de la session au fil du temps
