---
description: Configure un suscriptor edge para recibir feeds de shreds de DoubleZero, incluyendo configuración del cliente y reglas de firewall para GRE, BGP, PIM y tráfico de shreds.
---

# Conexión de Suscriptor Edge
!!! warning "Al conectarme a DoubleZero acepto los [Términos de Uso de DoubleZero](https://doublezero.xyz/terms-protocol). Tenga en cuenta que los datos son solo para sus propósitos internos y no pueden ser retransmitidos (consulte la Sección 2(e))."
...
- **SOL** — para las tarifas de transacción de Solana. Transfiera SOL a la dirección de wallet impresa arriba.
- **2Z** — para fondear el asiento. La CLI extrae de la Cuenta de Token Asociada (ATA) de su wallet para el mint de USDC en mainnet (`xfrsfrvsrf`).
...
Consulte los precios actuales del dispositivo antes de comprometer fondos. Los precios tienen dos componentes: un **precio base del metro** y una **prima por dispositivo**. También puede ver precios y disponibilidad [aquí](https://data.doublezero.xyz/dz/shreds/devices).
...
| `--amount <USDC>` | USDC a fondear (formato decimal, por ejemplo `100` = 100 USDC). Debe cumplir el precio mínimo por época. |
| `--source-token-account <PUBKEY>` | Cuenta fuente de USDC personalizada (por defecto usa la ATA de su wallet) |
| `--accept-partial-epoch` | Omitir la advertencia de época restante (ver abajo) |
| `--fee-payer <PATH>` | Usar una wallet diferente para las tarifas de transacción en SOL |
...
Los asientos se asignan por época de Solana (~2 días). Si queda menos del 10% de la época actual cuando paga, la CLI advierte que su asiento será asignado inmediatamente pero solo cubre el resto de la época actual. Se deducirá un pago separado de su escrow cuando comience la siguiente época.

!!! info "Es aconsejable fondear para más de 1 época a la vez para no perder su asiento. Puede verificar el tiempo restante en una época [aquí](https://explorer.solana.com/)."

Puede omitir esta advertencia con `--accept-partial-epoch`.

### Mantenga Su Escrow Fondeado

!!! warning "Si el saldo de su escrow está por debajo del precio de la época en el momento de la liquidación, su asiento no será asignado, el túnel será desactivado y perderá su antigüedad acumulada. La antigüedad determina su prioridad para épocas futuras — perderla significa que compite como un nuevo participante otra vez."

Puede sobrecargar esta cuenta para fondear múltiples épocas. Cada liquidación deduce el precio de una época de su escrow, y el saldo restante se traslada. Por ejemplo, fondear 5 veces el precio por época mantiene su asiento activo hasta por 5 épocas sin necesidad de re-fondear.
...
Esta sección detalla cómo ver asientos a través de la CLI. También puede usar [https://data.malbeclabs.com/api/v1/docs](https://data.doublezero.xyz/api/v1/docs) para monitorear asientos y ayudar a gestionar su cuenta de escrow.
...
Este comando libera su asiento y cierra el escrow. Usted recibe un reembolso prorrateado por la porción no utilizada de la época actual, más cualquier saldo restante del escrow, devuelto a su wallet. Pierde el asiento y toda la antigüedad acumulada.
...
Puede identificar el dispositivo ya sea con `--device <PUBKEY>` o `--device-code <CODE>`, igual que en otros comandos.
...
### [Tablero de Puntuación Edge](https://data.malbeclabs.com/dz/shreds/scoreboard)

El Tablero de Puntuación compara la velocidad de entrega de shreds entre DoubleZero Edge y otros proveedores, utilizando datos a nivel de slot para comparar el rendimiento en tiempo real. Use este panel para ver las tasas de victoria de los shreds de Edge contra otros proveedores. Puede ver resultados solo para shreds de líder, además de la comparación del feed completo. También puede desglosar por región para ver el rendimiento esperado.
...
Si el saldo de su escrow está por debajo del precio de la época en el momento de la liquidación, el asiento no se asigna, el túnel se desactiva y la antigüedad se pierde. Recargue con `shreds pay` antes de la próxima liquidación.

### Asiento no asignado después de pagar

- Es posible que haya pagado tarde en la época — el asiento toma efecto en la siguiente época.
...
- Esperar a la siguiente época para obtener cobertura de una época completa
