...
# 엣지 구독자 연결
!!! warning "DoubleZero에 연결함으로써 [DoubleZero 이용약관](https://doublezero.xyz/terms-protocol)에 동의합니다. 데이터는 내부 목적으로만 사용할 수 있으며 재전송할 수 없습니다(섹션 2(e) 참조)."

!!! warning "이미 CLI 구독을 사용 중이신가요?"
    **CLI**를 통해 구독한 경우(`doublezero-solana shreds pay` / 에스크로 시트), 해당 명령어는 [CLI 구독 페이지](Edge Subscriber CLI.md)를 참조하세요. 해당 시스템은 **2026년 8월 30일에 폐지**됩니다. 신규 구독은 이 페이지를 따르세요.
...
이전에 DoubleZero를 설정한 적이 있다면, `sudo apt update && sudo apt install doublezero-solana` 명령으로 최신 Doublezero-Solana CLI를 설치하세요.
...
가장 낮은 지연 시간 결과에서 메트로/도시를 확인합니다. 신청 양식에서 해당 도시를 선택하게 됩니다. 메트로가 어떻게 그룹화되어 있는지는 [토폴로지 맵](https://data.malbeclabs.com/topology/map?overlays=metroClustering%2Cbandwidth)을 참조하세요.

### 가격

시트는 선택한 메트로에서 머신당 **월별** 청구됩니다:
...
[계정](https://doublezero.xyz/shreds/account) 페이지에서 각 피드 요청에 DoubleZero ID(기존 키 또는 새로 생성)를 할당합니다. 일치하는 **개인 키는 슈레드를 수신할 머신에 있어야 합니다** — 해당 호스트로 이동할 수 없는 개인 키의 공개 키는 할당하지 마세요.

**메트로**와 **공개 키**를 선택합니다. 신청 시 공용 IP를 바인딩하지 **않습니다**. 구독 기간 동안 **선택한 메트로 내에서** IP 간에 접근을 이동할 수 있습니다.
...
신청서를 제출하면 인보이스를 받게 됩니다. 결제가 완료되면 승인된 각 머신에서 연결합니다. 선택한 시작일에 접근이 활성화됩니다.
...
리더 슈레드와 높은 스테이크 재전송 슈레드는 `doublezero1` 인터페이스를 통해 포트 `7733`으로 도착합니다. `doublezero0` 인터페이스는 유니캐스트 트래픽용입니다. 포트 `5765`는 슈레드 퍼블리셔의 하트비트 모니터로, 슈레드를 포함하지 않습니다.

슈레드 소비 시, **IP 주소**는 멀티캐스트 스트림을 식별하고 **포트**는 해당 스트림의 UDP 서비스를 식별합니다.  
아래 모든 슈레드 스트림은 `doublezero1`에서 UDP 포트 `7733`을 사용합니다.
...
스코어보드는 슬롯 수준 데이터를 사용하여 DoubleZero Edge와 다른 제공업체 간의 슈레드 전달 속도를 실시간으로 비교하여 벤치마킹합니다. 이 대시보드를 사용하여 다른 제공업체 대비 Edge 슈레드 승률을 확인할 수 있습니다. 리더 슈레드만의 결과와 전체 피드 비교를 모두 볼 수 있습니다. 또한 지역별로 드릴다운하여 예상 성능을 확인할 수 있습니다.

### [엣지 퍼블리셔](https://data.doublezero.xyz/dz/shreds/publishers)

대시보드 왼쪽 상단의 "Publishing Shreds" 지표는 DoubleZero Edge에서 리더 슈레드를 퍼블리싱하는 모든 Solana 검증자의 총 스테이크 가중치 비율을 보여줍니다. 네트워크의 각 퍼블리셔에 대한 세부 정보를 확인할 수 있습니다.

### [엣지 구독자, 디바이스 및 활동](https://data.doublezero.xyz/dz/shreds/subscribers)

이 페이지에서 클라이언트 IP를 검색하여 구독된 시트와 상태를 확인할 수 있습니다. 또한 [디바이스](https://data.doublezero.xyz/dz/shreds/devices) 페이지에서 사용 가능한 디바이스를 확인하고, [활동](https://data.doublezero.xyz/dz/shreds/activity) 페이지에서 모든 최근 활동을 확인할 수 있습니다.
...
여기에서 다루지 않는 문제가 발생하면, 직접 해결하기 전에 기존 채널을 통해 문의하세요. 채널이 없는 경우, [Discord](https://discord.gg/U2fEb4Jq)에서 검색하고 필요 시 티켓을 열어주세요.
