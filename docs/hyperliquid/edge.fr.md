...
Aperçu du service : [Hyperliquid](/hyperliquid/).
...
Vous voulez qu'une IA fasse l'installation avec vous ? Connectez le [MCP DoubleZero](/mcp/) et demandez-lui de vous guider à travers Hyperliquid Edge.
...
Suivez les instructions de [configuration](/setup/) pour installer et configurer le client DoubleZero sur l'hôte.
...
Les flux sont tarifés par région de livraison. Le prix dépend de l'endroit où les données sont livrées, et non de l'emplacement de l'acheteur. Un forfait Tokyo livre aux récepteurs à Tokyo ; la livraison ailleurs nécessite le forfait Global. Deux hôtes récepteurs (IP) sont inclus par flux, par métro.
...
Vous attribuerez un identifiant DoubleZero (clé existante ou générez-en une nouvelle) à chaque demande de flux sur la page [comptes](https://doublezero.xyz/shreds/account). La **clé privée correspondante doit être présente sur la machine qui recevra le flux** — n'attribuez pas une clé publique dont vous ne pouvez pas déplacer la clé privée vers cet hôte.
...
Si un `doublezerod` hôte est déjà en cours d'exécution (depuis la [configuration](/setup/)), arrêtez-le d'abord — il entre en conflit avec le démon du conteneur pour le même tunnel :

```bash
sudo systemctl stop doublezerod
```

Installez [doublezero-edge-connect](https://github.com/malbeclabs/doublezero-edge-connect) **après** l'approbation et le paiement. Le bridge rejoint DoubleZero à l'intérieur d'un conteneur `--network host` et sert du JSON normalisé sur `ws://<host>:8081`.
...
Attendez `BGP Session Up` et vos groupes `edge-hyper-…` abonnés.

Ensuite, ouvrez le WebSocket (`ws://127.0.0.1:8081`). Contrat : [PROTOCOL.md](https://github.com/malbeclabs/doublezero-edge-connect/blob/main/PROTOCOL.md). Guide complet : runbook [MCP](/mcp/) `hyperliquid-edge`.

### 4b. Multicast natif

Sur l'hôte qui détient la clé privée assignée (avec le `doublezerod` hôte en cours d'exécution), abonnez-vous aux flux que vous avez achetés :
...
Attendez `BGP Session Up` sur le bon réseau DoubleZero. Ensuite, décodez le fil vous-même — voir [Décoder le flux](#decode-the-feed).
...
L'IP détermine le groupe multicast. Le port détermine le flux sur ce groupe. Vérifiez les valeurs IP en temps réel avec :
...
Les trames sont en binaire little-endian de taille fixe. Les perps natifs Hyperliquid utilisent `source_id=1` ; les perps trade.xyz utilisent `source_id=7`.
...
!!! note "Edge Connect"
    Si vous utilisez `doublezero-edge-connect`, le flux est déjà décodé en JSON via WebSocket — passez le décodage manuel.
...
Si vous rencontrez un problème non couvert ici, veuillez nous contacter via votre canal existant avant de chercher une solution de contournement. Si vous n'avez pas de canal, consultez [Support](/support/).
...
1. **Edge Connect :** exécutez le statut dans le conteneur — `docker exec doublezero-edge-connect doublezero status`. Le `doublezero status` de l'hôte échoue souvent alors que le flux fonctionne bien (le conteneur possède le démon). Confirmez que le `doublezerod` de l'hôte est arrêté.
...
4. Confirmez que la facture pour ce siège est payée et que la date de début est passée
5. Exécutez connect sur le chemin que vous avez choisi ([4a](#4a-edge-connect) ou [4b](#4b-native-multicast)) avec la clé correspondant à la page des comptes
6. Attendez `BGP Session Up` depuis le même endroit où vous avez exécuté connect (conteneur ou hôte)

**Aucun paquet après l'abonnement**
...
4. Privilégiez la liaison du marché et de la référence ensemble (et du snapshot pour MBO) pour le flux souhaité
...
Ensuite, réessayez `doublezero connect multicast --subscribe-feed <feed-code>` sur le même chemin (conteneur ou hôte).
