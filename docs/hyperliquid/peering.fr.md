...
Plus d'informations sur le peering sont disponibles via [Telegram](https://t.me/doublezero_telegram_bot?start=fromwebsite). Incluez l'IP publique statique de votre nœud (ainsi que la région). Comptez un délai de **1 à 3 jours ouvrés** pour recevoir les détails de peering après réception des informations de votre nœud et finalisation du paiement.

## Pourquoi le peering est payant

Les pairs racine publics d'Hyperliquid sont partagés : les slots sont disputés, les pairs changent ou imposent des limites de débit, et beaucoup ne relaient pas le mempool. Un slot de peering réservé vous offre un upstream stable sur DoubleZero au lieu de rivaliser pour cette capacité publique.
...
- Les clients se connectent à un niveau de Block Proxy qui évolue horizontalement. Chaque proxy se connecte à nos deux nœuds non-validateurs et apparaît comme un pair de gossip ordinaire pour chacun d'eux.
...
Le nœud de la Hyper Foundation se connecte à notre nœud primaire ; une sentinelle alimente notre nœud secondaire. Chaque Block Proxy se connecte aux deux, fusionne leurs flux pour ses clients et récupère les snapshots de démarrage depuis le service de snapshots lorsque nécessaire.

## Disponibilité

Objectif : 99,9 % de disponibilité mensuelle.

- Flux redondant. Chaque proxy se connecte à nos deux nœuds non-validateurs. Ces nœuds reçoivent les blocs de sources indépendantes (Foundation et sentinelle). Si une session ou une source tombe, l'autre maintient le flux de blocs pendant la récupération du chemin défaillant.
- Nos nœuds restent derrière le niveau de proxy. Les pairs externes n'atteignent que les Block Proxies. Si un proxy échoue, ses pairs se reconnectent à un autre proxy sain. La charge ne retombe jamais sur nos nœuds.
- Les proxies ne conservent que des caches jetables (snapshot, fenêtre glissante de blocs, tampons en direct), pas l'état autoritaire de la chaîne. Un proxy défaillant est remplacé automatiquement. Un remplaçant n'entre en service qu'après que les sessions upstream et les caches ont passé les vérifications de disponibilité. Nos nœuds non-validateurs ne sont pas redémarrés pour cela.
...
- Un pair qui rejoint prend son snapshot de démarrage depuis le service de snapshots et les blocs de rattrapage depuis le propre stockage du proxy, jamais depuis nos nœuds non-validateurs. Une vague de nouveaux pairs impacte le niveau de proxy à la place.

## Tarification

999 $/mois pour le peering. Inclut le mempool. N'inclut pas les flux de données de marché Edge. Un hôte récepteur (IP) est inclus. Le peering n'est pas regroupé ni remisé avec les données de marché Edge.