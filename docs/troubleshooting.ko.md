---
description: 일반적인 DoubleZero 연결 문제를 참조 명령어, 예상 출력 및 추가 지원을 받을 수 있는 곳을 통해 진단합니다.
---
...
먼저 다음 명령어의 출력과 예상 출력을 확인하세요. 이것들은 더 자세한 문제 해결에 도움이 됩니다.
티켓을 열 경우, 해당 출력을 요청받을 수 있습니다.
...
샘플 공개키: `MTAwoHgKyTwwDGJo2dye6EWqyTn27JRwXxaDEaeMqe2` 명령어 실행 시 이것을 본인의 공개키로 대체하세요.

명령어:

`doublezero access-pass list | grep MTAwoHgKyTwwDGJo2dye6EWqyTn27JRwXxaDEaeMqe2`

출력: [참고: 이 출력에서 헤더를 함께 표시하기 위해 `doublezero access-pass list | awk 'NR==1 || /MTAwoHgKyTwwDGJo2dye6EWqyTn27JRwXxaDEaeMqe2/'`를 사용합니다]
```
...
# 문제 해결 예시
기본 출력과 정상 배포에서 예상되는 결과를 살펴보았으므로, 이제 몇 가지 일반적인 문제 해결 예시를 살펴보겠습니다.

### 문제: ❌ Error creating user

이 문제는 일반적으로 예상되는 공개키/IP 쌍과 사용자가 DoubleZero에 접속하려는 공개키/IP 쌍 간의 불일치와 관련됩니다.

**증상:**
- `doublezero connect ibrl`로 연결할 때 사용자에게 `❌ Error creating user`가 발생합니다
...
3. 이 오류는 특정 DoubleZero IP를 사용하는 DoubleZero 터널이 이미 이 머신에서 활성화되어 있기 때문에 나타납니다.
    ...
    이 오류는 DoubleZero 클라이언트 업그레이드 후에 자주 발생합니다. DoubleZero 업그레이드는 doublezerod 서비스를 자동으로 재시작하며, 서비스 재시작 전에 연결되어 있었다면 자동으로 재연결합니다.


### 문제: DoubleZero 상태가 unknown 또는 down
이 문제는 서버와 DoubleZero 장치 간의 GRE 터널은 성공적으로 활성화되었지만, 방화벽이 BGP 세션 설정을 차단하는 경우와 관련이 있습니다. 이로 인해 네트워크에서 라우트를 수신하거나 DoubleZero를 통해 트래픽을 전송할 수 없습니다.
...
1. 방화벽 규칙을 확인하세요!
   ...
   DoubleZero는 머신과 DoubleZero 장치 간의 GRE 터널 인터페이스에 링크 로컬 주소 공간 169.254.0.0/16을 사용합니다. 169.254.0.0/16은 일반적으로 "라우팅 불가" 공간이므로 좋은 보안 관행에서는 이 공간으로의/으로부터의 통신을 차단하도록 권장합니다. 방화벽에서 src 169.254.0.0/16이 dst 169.254.0.0/16과 tcp 포트 179에서 통신할 수 있도록 허용하는 규칙을 추가해야 합니다. 이 규칙은 169.254.0.0/16으로의 트래픽을 거부하는 규칙보다 위에 배치되어야 합니다.

    ufw와 같은 방화벽에서 `sudo ufw status`를 실행하여 방화벽 규칙을 확인할 수 있습니다.

    Solana 밸리데이터가 사용할 수 있는 것과 유사한 샘플 출력입니다.
    ```
    To                         Action      From
    --                         ------      ----
    ...
    위의 출력에서 지정된 포트를 제외한 169.254.0.0/16으로의 모든 트래픽이 거부되는 것을 볼 수 있습니다.
    `sudo ufw insert <N> allow proto tcp from 169.254.0.0/16 to 169.254.0.0/16 port 179`를 사용하여 <N> 위치에 규칙을 삽입합니다. 예: N = 1이면 이 규칙을 첫 번째 규칙으로 삽입합니다.
    `sudo ufw status numbered`를 실행하면 규칙의 번호 순서를 확인할 수 있습니다.
    
### 문제: 가장 가까운 DoubleZero 장치가 변경됨

이것은 오류가 아니라 최적화를 위한 것입니다. 아래는 수시로 실행하거나 자동화할 수 있는 모범 사례입니다.
...
2. 이미 대상 장치에 연결되어 있는지 확인
    - `doublezero user list --env testnet | grep 111.11.11.11` 실행 - `111.11.11.11`을 DoubleZero에 연결된 장치의 공용 IPv4 주소로 대체하세요. 밸리데이터 ID 또는 doublezero ID를 사용할 수도 있습니다.
        ...
        ```
        이 예시에서는 이미 가장 가까운 장치에 연결되어 있습니다. 더 이상의 단계가 필요하지 않으며, 여기서 멈출 수 있습니다.


        대신 출력이 다음과 같은 경우를 고려해 보겠습니다
         ```
        account                                      | user_type           | groups                        | device       | location    | cyoa_type  | client_ip       | dz_ip           | accesspass                                                      | tunnel_id | tunnel_net       | status    | owner                                        
        ...
        ```
        이것은 최적이 아닌 연결입니다. 재연결이 필요한지 고려해 보겠습니다.

        연결 전에 해당 장치에 사용 가능한 사용자 터널이 있는지 확인합니다.
...
3. 선택 사항: 네트워크에서 사용 가능한 장치 확인
    ...
    - `doublezero device list`를 실행하여 전체 장치 목록을 확인합니다. 출력을 설명하기 위해 2개의 장치를 예시로 가져왔습니다.
        ...
        그러나 `dz-fr5-sw01`은 0명의 사용자와 0명의 최대 사용자가 있습니다. 이 장치에는 연결할 수 없습니다. 최대 사용자가 0이므로 장치가 어떤 연결도 수락하지 않습니다.
...
4. 대상 장치에 사용 가능한 연결이 있는지 확인
    - `doublezero device list | grep dz-ny7-sw01` 실행 - `dz-ny7-sw01`을 대상 장치로 대체하세요
        ...
        ```
        여기서 `dz-ny7-sw01`에 연결할 수 있는 여유 공간이 있음을 확인할 수 있습니다.
...
5. 가장 가까운 DoubleZero 장치에 연결
    ...
    ✅  User Provisioned
    ```
    위의 출력에서 `Connected to device: dz-ny7-sw01`에 연결된 것을 확인할 수 있습니다. 이것은 1단계의 초기 조사에서 `dz-ny7-sw01`이 가장 낮은 지연 시간을 가진 장치임을 발견한 결과로 원하는 결과입니다.

### 문제: `doublezero status`가 일부 필드에 N/A를 반환함

이 문제는 일반적으로 현재 데몬 및 클라이언트와, 연결된 DZ 터널이 설정된 시점의 데몬 및 클라이언트 간의 불일치와 관련됩니다.

**증상:**
- `doublezero status`를 실행할 때 사용자가 일부 필드에서 `N/A`를 발견합니다
...
`doublezero status`
    ...
    위의 예시 출력에서 `Tunnel status`가 `up`인 것을 확인할 수 있습니다. `Network`는 `mainnet-beta`입니다. 그러나 `Current Device`와 `Metro`는 `N/A`입니다.

    이것은 현재 환경에 없는 머신에서 열린 터널이 있음을 나타냅니다.
    이 경우 `up` 상태이지만 `mainnet-beta`에서 `Current Device`를 찾을 수 없다는 것은 터널이 testnet에 있다는 것을 알려줍니다!
 
2. 환경을 변경합니다.

    불일치를 해결하기 위해 `N/A`를 반환하는 환경의 반대 환경