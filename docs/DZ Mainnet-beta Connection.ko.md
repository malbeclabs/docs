---
description: Solana 메인넷-베타 검증자와 최대 3개의 백업을 IBRL 모드로 DoubleZero에 연결하며, 신원 증명 및 연결 요청을 포함합니다.
---

# 검증자 메인넷-베타 IBRL 모드 연결
!!! warning "DoubleZero에 연결함으로써 [DoubleZero 서비스 약관](https://doublezero.xyz/terms-protocol)에 동의합니다"



### 메인넷-베타에 IBRL 모드로 연결하기

!!! Note inline end
    IBRL 모드는 기존 공용 IP 주소를 사용하므로 검증자 클라이언트를 재시작할 필요가 없습니다.

Solana 메인넷 검증자는 이 페이지에서 자세히 설명하는 DoubleZero 메인넷-베타에 대한 연결을 완료합니다.

각 Solana 검증자는 고유한 **신원 키쌍**을 가지고 있습니다. 여기서 **노드 ID**로 알려진 공개 키를 추출합니다. 이것이 Solana 네트워크에서 검증자의 고유 식별자입니다.

DoubleZeroID와 노드 ID가 확인되면, 머신의 소유권을 증명합니다. 이는 검증자의 신원 키로 서명된 DoubleZeroID를 포함하는 메시지를 생성함으로써 수행됩니다. 생성된 암호화 서명은 검증자를 제어하고 있다는 검증 가능한 증명 역할을 합니다.

마지막으로, **DoubleZero에 연결 요청**을 제출합니다. 이 요청은 다음을 전달합니다: *"여기에 내 신원이 있고, 여기에 소유권 증명이 있으며, 여기에 내가 연결하려는 방법이 있습니다."* DoubleZero는 이 정보를 검증하고, 증명을 수락하며, DoubleZero에서 검증자의 네트워크 접근을 프로비저닝합니다.

이 가이드는 1개의 기본 검증자를 등록하고, 동시에 최대 3개의 백업/장애 조치 머신을 등록할 수 있습니다.

## 사전 요구 사항

- Solana CLI가 설치되어 있고 $PATH에 등록되어 있어야 합니다
- 검증자의 경우: sol 사용자 하에서 검증자 신원 키쌍 파일(예: validator-keypair.json)에 대한 접근 권한
- 검증자의 경우: 연결하려는 Solana 검증자의 신원 키에 최소 1 SOL이 있는지 확인
- 방화벽 규칙이 DoubleZero 및 Solana RPC에 필요한 아웃바운드 연결을 허용해야 하며,
 GRE (ip proto 47) 및 BGP (169.254.0.0/16 on tcp/179) 포함

!!! info
    검증자 ID는 Solana 가십을 통해 대상 IP를 결정하기 위해 확인됩니다. 대상 IP와 DoubleZero ID는 머신과 대상 DoubleZero 장치 간의 GRE 터널을 열 때 사용됩니다.

    참고: 동일한 IP에 임시 ID와 기본 ID가 있는 경우, 기본 ID만 머신 등록에 사용됩니다. 이는 임시 ID가 가십에 나타나지 않으므로 대상 머신의 IP를 확인하는 데 사용할 수 없기 때문입니다.

## 1. 클라이언트 네트워크 확인

진행하기 전에 [설정](setup.md) 지침을 따르십시오. **메인넷-베타** 패키지를 설치하십시오 — 테스트넷과 메인넷-베타는 다른 패키지 저장소를 사용합니다.

설정의 마지막 단계는 네트워크 연결을 해제하는 것이었습니다. 이는 머신에서 DoubleZero로의 터널이 하나만 열려 있고, 해당 터널이 올바른 네트워크에 있는지 확인하기 위함입니다.

클라이언트가 mainnet-beta에 있는지 확인합니다:

```bash
doublezero status
```

`Network` 열이 `mainnet-beta`여야 합니다. `testnet`이거나 잘못된 패키지를 설치한 경우, [문제 해결](troubleshooting.md#issue-wrong-doublezero-environment)의 복사-붙여넣기 전환을 사용하십시오.

약 30초 후 사용 가능한 DoubleZero 장치를 볼 수 있습니다:
...
일부 [라우팅 기능](https://github.com/malbeclabs/doublezero/blob/main/rfcs/rfc7-client-route-liveness.md)을 활용하려면 포트 44880을 열어야 합니다.
...
이 규칙을 DoubleZero 인터페이스에만 제한하는 `-i doublezero0`, `-o doublezero0` 플래그에 유의하십시오

또는 다음과 같이 UFW를 사용할 수 있습니다:
...
이 규칙을 DoubleZero 인터페이스에만 제한하는 `in on doublezero0`, `out on doublezero0` 플래그에 유의하십시오

## 3. 검증자 소유권 증명

<div data-wizard-step="mainnet-find-validator" markdown>

DoubleZero 환경이 설정되었으므로, 이제 검증자 소유권을 증명할 차례입니다.

기본 검증자의 [설정](setup.md)에서 생성한 DoubleZero ID는 모든 백업 머신에서 사용해야 합니다.

기본 머신의 ID는 `doublezero address`로 확인할 수 있습니다. 동일한 ID가 클러스터의 모든 머신에서 `~/.config/doublezero/id.json`에 있어야 합니다.

이를 수행하려면 먼저 명령을 실행하는 머신이 **기본 검증자**인지 다음으로 확인합니다:

```
doublezero-solana passport find-validator -u mainnet-beta
```

이것은 검증자가 가십에 등록되어 있고 리더 스케줄에 나타나는지 확인합니다.
...
!!! info
    하나 또는 여러 머신에 대해 동일한 워크플로가 사용됩니다.
    하나의 머신만 등록하려면 이 페이지의 모든 명령에서 "--backup-validator-ids" 또는 "backup_ids=" 인수를 제외하십시오.

이제 **기본 검증자**를 실행할 예정인 모든 백업 머신에서 다음을 실행합니다:
```
doublezero-solana passport find-validator -u mainnet-beta
```
...
이제 **기본 검증자** 투표 계정과 신원을 사용할 예정인 **모든 백업 머신**에서 이 명령을 실행합니다.
...
**기본 검증자** 머신에서 다음 명령을 실행합니다. 이것은 활성 스테이크가 있고, 리더 스케줄에 있으며, 명령을 실행하는 머신의 solana 가십에 기본 검증자 ID가 있는 머신입니다:
...
```
이 명령의 마지막 출력을 확인하십시오. 이것이 다음 단계의 구조입니다.
...
마지막 단계에서 `solana sign-offchain-message`에 대한 사전 형식화된 출력을 받았습니다.

위의 출력에서 **기본 검증자** 머신에서 이 명령을 실행합니다.
...
`request-validator-access` 명령을 사용하여 연결 요청을 위한 Solana 계정을 생성합니다. DoubleZero Sentinel 에이전트가 새 계정을 감지하고, 신원과 서명을 검증한 후, DoubleZero에서 접근 패스를 생성하여 서버가 연결을 설정할 수 있도록 합니다.


노드 ID, DoubleZeroID, 서명을 사용합니다.

!!! note inline end
      이 예시에서는 검증자 신원을 찾기 위해 `-k /home/user/.config/solana/id.json`을 사용합니다. 로컬 배포에 적합한 경로를 사용하십시오.
...
이 출력은 Solana 탐색기에서 트랜잭션을 확인하는 데 사용할 수 있습니다. 탐색기를 메인넷으로 변경해야 합니다. 이 검증은 선택 사항입니다.

```bash
Request Solana validator access: Transaction22222222VaB8FMqM2wEBXyV5THpKRXWrPtDQxmTjHJHiAWteVYTsc7Gjz4hdXxvYoZXGeHkrEayp
```

성공하면 DoubleZero가 기본 검증자와 백업을 등록합니다. 이제 접근 패스에 등록된 IP 간에 장애 조치가 가능합니다. DoubleZero는 이 방식으로 등록된 백업 노드로 전환할 때 자동으로 연결을 유지합니다.
...
✅  User Provisioned
```
GRE 터널 설정이 완료될 때까지 1분간 기다리십시오. GRE 터널 설정이 완료되기 전까지 상태 출력이 "down" 또는 "Unknown"을 반환할 수 있습니다.
...
**출력:**
!!! note inline end
    이 출력을 확인하십시오. `Tunnel src`와 `DoubleZero IP`가 머신의 공용 IPv4 주소와 일치하는 것에 주목하십시오.
...
다음을 실행하여 DoubleZero의 다른 사용자가 전파한 라우트를 확인할 수 있습니다:
...
### 다음 단계: 멀티캐스트를 통한 슈레드 퍼블리싱

이 설정을 완료했고 멀티캐스트를 통해 슈레드를 퍼블리싱할 계획이라면, [다음 페이지](Validator%20Multicast%20Connection.md)로 진행하십시오.