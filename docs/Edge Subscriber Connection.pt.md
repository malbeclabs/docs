---
description: Configure um assinante de borda para receber feeds de shreds DoubleZero, incluindo configuração do cliente e regras de firewall para GRE, BGP, PIM e tráfego de shreds.
---
...
O reconciliador monitora o estado onchain e provisiona túneis automaticamente quando seu assento é alocado. Ele não está habilitado por padrão.
...
- **2Z** — para financiamento de assentos. O CLI retira da Associated Token Account (ATA) da sua carteira para o mint USDC da mainnet (`xfrsfrvsrf`).
...
Consulte os preços atuais dos dispositivos antes de comprometer fundos. Os preços têm dois componentes: um **preço base do metro** e um **prêmio por dispositivo**. Você também pode ver preços e disponibilidade [aqui](https://data.doublezero.xyz/dz/shreds/devices).
...
O preço por época é o custo total por época para um assento nesse dispositivo (base + prêmio). Use `--wide` para mostrar chaves públicas completas, ou `--json` para saída em JSON.
...
Quando seu assento for alocado, o daemon estabelece o túnel GRE automaticamente. Verifique sua conexão com:
...
Os assentos são alocados por época Solana (~2 dias). Se menos de 10% da época atual restar quando você pagar, o CLI avisa que seu assento será alocado imediatamente, mas cobre apenas o restante da época atual. Um pagamento separado será deduzido do seu escrow quando a próxima época começar.

!!! info "É aconselhável financiar mais de 1 época por vez para que você não perca seu assento. Você pode verificar o tempo restante de uma época [aqui](https://explorer.solana.com/)."

Você pode ignorar este aviso com `--accept-partial-epoch`.

### Manter Seu Escrow Financiado

!!! warning "Se o saldo do seu escrow estiver abaixo do preço da época na liquidação, seu assento não será alocado, o túnel será encerrado e você perderá sua antiguidade acumulada. A antiguidade determina sua prioridade para épocas futuras — perdê-la significa que você compete como um novato novamente."

Você pode financiar esta conta a mais para cobrir múltiplas épocas. Cada liquidação deduz o preço de uma época do seu escrow, e o saldo restante é transferido. Por exemplo, financiar 5x o preço por época mantém seu assento ativo por até 5 épocas sem refinanciamento.
...
Esta seção detalha como visualizar assentos via CLI. Você também pode usar [https://data.malbeclabs.com/api/v1/docs](https://data.doublezero.xyz/api/v1/docs) para monitorar assentos e auxiliar no gerenciamento da sua conta de escrow.
...
A coluna "Est. Epochs Paid" mostra quantas épocas seu saldo atual cobre nos preços atuais. Se os preços mudarem, esta estimativa se ajusta.

### Retirar Assento e Escrow

Este comando libera seu assento e fecha o escrow. Você recebe um reembolso proporcional pela porção não utilizada da época atual, mais qualquer saldo restante do escrow, devolvido à sua carteira. Você perde o assento e qualquer antiguidade acumulada.
...
Para enviar o reembolso em USDC para uma conta de token diferente:
...
!!! warning "Isso não pode ser desfeito. Após a retirada, seu assento é perdido e a antiguidade é redefinida."

---

## Endereços de Shreds (IP vs Porta)

Shreds de Líder e Shreds de Retransmissão de alto stake chegarão pela porta `7733`, através da interface `doublezero1`. A interface `doublezero0` é para tráfego unicast. A porta `5765` é um monitor de heartbeat dos publicadores de shreds — ela não conterá shreds.
...
!!! note "O tráfego de shreds entregue pela rede é encapsulado em GRE. Você pode precisar remover o cabeçalho GRE antes de alimentar os dados no seu pipeline existente (ex.: um deshredder baseado em XDP)."
...
Se o saldo do seu escrow estiver abaixo do preço da época na liquidação, o assento não é alocado, o túnel é encerrado e a antiguidade é perdida. Recarregue com `shreds pay` antes da próxima liquidação.

### Assento não alocado após o pagamento

- Você pode ter pago tarde na época — o assento entra em vigor na próxima época.
- Todos os assentos no dispositivo podem estar ocupados por incumbentes com maior antiguidade. Verifique os assentos disponíveis com `shreds price`.
- Se você retirou antes da liquidação, o assento não era elegível.

### Túnel não está sendo estabelecido
...
Você já possui uma assinatura ativa por um caminho diferente. Desconecte primeiro com `doublezero disconnect`, depois tente novamente `shreds pay`.