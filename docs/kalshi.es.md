...
Dos caminos. Prefiere Edge Connect a menos que necesites controlar el decodificador.

| # | Camino | Ideal para | Esfuerzo |
|---|--------|------------|----------|
| **1** | [Edge Connect](#1-edge-connect-recomendado) | Agentes y aplicaciones que desean un CLI sencillo y un WebSocket JSON normalizado | Más bajo |
| **2** | [Multicast nativo](#2-multicast-nativo-avanzado) | Construir tu propio decodificador contra el formato de cable crudo | Más alto |
...
**Comienza aquí.** [doublezero-edge-connect](https://github.com/malbeclabs/doublezero-edge-connect) es el camino amigable para agentes: un solo comando de instalación, el host se une a DoubleZero, y tu aplicación consume **JSON normalizado sobre WebSocket** (`ws://<host>:8081`) en lugar de decodificar multicast binario.

El equipo evoluciona Edge Connect para satisfacer las necesidades de su base de usuarios en expansión. Este es el método de conexión más fácil y debe usarse a menos que tengas una necesidad técnica específica.
...
`DZ_SECRET` es un token de acceso `DZ_…` **o** la ruta al JSON del keypair de Solana que posee tu pase de acceso / compra del feed.

Si ya hay un `doublezerod` del host en ejecución, detenlo primero — compite con el daemon del contenedor por el mismo túnel:
...
!!! warning "Se requiere conocimiento técnico avanzado"
    Multicast nativo significa que te unes al grupo tú mismo y decodificas el formato de cable **crudo** de Edge en tu host. Solo los usuarios con mayor capacidad técnica deberían tomar este camino. Necesitarás leer y comprender las especificaciones, comenzando por [market-by-price/spec.md](https://github.com/malbeclabs/edge-feed-spec/blob/main/market-by-price/spec.md) y el resto de [edge-feed-spec](https://github.com/malbeclabs/edge-feed-spec). Prefiere [Edge Connect](#1-edge-connect-recomendado) a menos que tengas un requisito estricto de controlar el decodificador.
...
# Kalshi mercado / referencia / snapshot (todos los feeds)
...
Espera `BGP Session Up` en la red DoubleZero correcta. Como suscriptor, tu IP de DoubleZero coincide con tu IP de Tunnel Src.
...
La versión del esquema es **`3`** — descarta los frames cuya versión tu decodificador no implemente. Layouts autorizados: [edge-feed-spec](https://github.com/malbeclabs/edge-feed-spec), incluyendo [market-by-price/spec.md](https://github.com/malbeclabs/edge-feed-spec/blob/main/market-by-price/spec.md).

Cada datagrama comienza con un encabezado de frame, seguido por uno o más mensajes de aplicación empaquetados hasta el MTU. Los frames son little-endian y de layout fijo.

| Campo | Notas |
|-------|-------|
| Versión del esquema | `3` |
| ID del canal | Demultiplexa flujos que comparten un puerto |
| Secuencia | Monótona por canal — úsala para detectar huecos |
| Marca de tiempo de envío | Nanosegundos desde la época Unix |
| Conteo de mensajes | Mensajes empaquetados en este frame |
| Conteo de reinicios | Avanza por sesión. Un incremento significa reiniciar tu estado en frío. |
| Longitud del frame | Total de bytes |

#### Mensajes de aplicación (TOB)

| Tipo | ID | Tamaño | Puerto | Contenido |
|------|----|--------|--------|-----------|
| Heartbeat | `0x01` | 16 B | market | Señal de actividad mientras el mercado está en calma |
...
| ManifestSummary | `0x07` | 24 B | reference | Huella del conjunto activo y conteo de instrumentos |
| PerpStats | `0x30` | 124 B | sibling | Financiamiento, precios mark y oracle, interés abierto, volumen diario |

El ID de fuente de Kalshi en el registro de edge-feed-spec es `3`. Lee `price_exponent` y `qty_exponent` de cada `InstrumentDefinition` — no los codifiques de forma fija.
...
La entrega es UDP de tipo fire-and-forget sin retransmisión. Recupera los datagramas perdidos del ciclo de reference-data (y del plano de snapshot en los feeds MBP), que se re-emite de forma periódica en lugar de una sola vez.
...
| `edge-kalshi-sports-tob` | Deportes top-of-book | `233.84.178.17` | `33000` + id | `43000` + id | — |
| `edge-kalshi-sports-mbp` | Deportes market-by-price | `233.84.178.20` | `34000` + id | `44000` + id | `54000` + id |
...
El grupo selecciona el feed; el puerto selecciona datos de mercado, datos de referencia o snapshot dentro de él. La replicación multicast ocurre por fuente y grupo, y la red nunca inspecciona el puerto UDP, por lo que unirse a un grupo entrega todo lo que hay en ese grupo a través de tu enlace Edge Connect. El puerto es un filtro de socket aplicado en tu propio host después de que los bytes llegan.
...
5. Confirma que el firewall permite los puertos UDP del feed entrantes en `doublezero1`.

### Huecos de secuencia

La secuencia es monótona por canal. Un hueco significa datagramas perdidos; el siguiente ciclo de reference-data restaura el estado del instrumento.

### Los frames se detienen y luego reinician con un nuevo conteo de reinicios

Un reinicio del publicador avanza el conteo de reinicios en el encabezado del frame. Descarta el estado de la sesión anterior y reinicia en frío desde el siguiente ciclo de reference-data.
...
4. Verifica el estado de conexión desde el mismo lugar donde te conectaste (contenedor o host) — espera `BGP Session Up` en la red DoubleZero correcta
...
Apunta `.env` a tu grupo de Kalshi y puertos (ver [Direcciones de feeds](#direcciones-de-feeds)), luego:
...
Esto visualiza los datos que ya estás recibiendo. No reemplaza la compra del feed, la suscripción ni ninguno de los caminos de conexión anteriores.